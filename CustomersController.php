...
use App\Models\Core\Corporate;
use App\Models\Core\Images;
use App\Models\Core\Setting;
use App\Models\Core\Languages;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Jobs\WelcomeNewCustomer;
use App\Mail\WelcomeNewCustomersMail;
use App\Traits\NotificationConfigHandler;
...
{
    use NotificationConfigHandler;
    public function __construct(Customers $customers, Setting $setting, Corporate $corporates, Customer $customer)
    {
    ...
    public function display()
    {
        ...
        $corporates = $this->corporates->getCorporates();
        ...
        $customerData['corporates'] = $corporates;
    ...
    {
        return $this->customer->processCorporateAccounts($request);
    }
    ...
    private function welcomeMessage($customer)
    {
        return "Dear ".ucfirst($customer->first_name).". Thank you for registering with Shopsavvy. Your account login details have been sent to your e-mail. Happy Shopping!!";
    }
    ...
    public function insert(Request $request)
    {
        ...
        if (count($existEmail)> 0 ) {
            ...
            $customer = $this->Customers->insert($request);

            \Mail::to($customer->email)
            ->send(new WelcomeNewCustomersMail($customer, $request->password));
             $this->messangerConfig($customer->phone, $this->welcomeMessage($customer));
            //dispatch(new WelcomeNewCustomer($customer));
            return redirect('admin/customers/address/display/' . $customer->id)->with('update', 'Customer has been created successfully!');
        }
    }
